...
const qimenParameters = Object.freeze({
  type: 'object',
  properties: {
    scope: { type: 'string', enum: ['hour', 'day', 'month', 'year'], description: '当下行动用 hour，某日用 day，月度用 month，年度用 year。' },
...
const huangjiParameters = Object.freeze({
  ...
  properties: {
    ...
    year: { type: 'integer', minimum: 1900, maximum: 2199 },
  },
  required: ['mode'],
  additionalProperties: false,
});
const agentTools: AgentToolDefinition[] = [
  ...
  { name: 'cast_qimen', description: '调用奇门遁甲。适合行动时机、方位、出行、布局、谈判、竞争、寻找失物、决策策略；支持时日月年局、转飞盘和拆补置闰。', parameters: qimenParameters },
  { name: 'cast_liuren', description: '调用大六壬。适合人物关系复杂、过程多变的现实事件，尤其商业合作、官非纠纷、职场博弈、多人事件和事情来龙去脉。', parameters: emptyParameters },
  { name: 'cast_taiyi', description: '调用太乙神数年、月、日、时四计。仅用于明确提到太乙，或宏观群体、地区与大势问题，不用于普通个人小事。', parameters: taiyiParameters },
  ...
  { name: 'calculate_huangji_jingshi', description: '调用皇极经世值年卦或年月日时盘。只在用户明确提到皇极经世、值年卦、元会运世或月日时卦时使用。', parameters: huangjiParameters },
...
function selectionFromCall(call: { name: AgentToolName; arguments: Record<string, unknown> }) {
  ...
  if (call.name === 'calculate_huangji_jingshi') {
    ...
    return {
      ...
      huangjiMode,
      ...(huangjiMode === 'year' ? { huangjiYear: Number.isInteger(year) && year >= 1900 && year <= 2199 ? year : new Date().getFullYear() } : {}),
    };
  }
