...
import { fortuneStatusMeta, type FortuneStatus } from '../lib/fortuneStatus';
...
<template>
  <figure class="fortune-status-image" :class="[`tone-${meta.tone}`, { compact }]">
    <img :src="meta.image" :alt="`${meta.label}状态图`" />
  </figure>
</template>

<style scoped>
.fortune-status-image { margin: 0; min-width: 0; }
.fortune-status-image img { display: block; height: auto; max-height: 100%; object-fit: contain; width: 100%; }
