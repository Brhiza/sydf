...
function chooseActivePromptSchool(event: Event) {
  const method = activePromptSchoolMethod.value;
  const target = event.target;
  if (!method || !(target instanceof HTMLSelectElement)) return;
  choosePromptSchool(method, target.value);
}
      ...
      <main ref="contentRef" class="content">
        <UiPageShell v-if="activeView === 'tools'" width="reading" class="screen tools-screen" :class="{ 'is-chat': homeState === 'chat' }">
          ...
          <div v-if="!chatSelectionMode" class="chat-composer chat-composer-docked" :class="{ 'home-default-composer': homeState === 'default' }">
            ...
            <label v-if="activePromptSchoolMethod" class="setting-row prompt-school-row"><span>解读流派</span><select :value="activePromptSchoolChoice" aria-label="选择解读流派" @change="chooseActivePromptSchool"><option v-for="item in activePromptSchoolOptions" :key="item.value" :value="item.value">{{ item.label }}</option></select></label>
