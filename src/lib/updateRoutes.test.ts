...
describe('APK 多线路下载', () => {
  it('按蓝奏云、Release、加速线路和 R2 的顺序生成五条官方线路', () => {
    ...
    expect(routes[0]?.url).toBe('https://lanzou-cloudflare-api.brhiza.workers.dev/v1/public/shiyue-dongfang/0.2.0');
    expect(routes[1]?.url).toBe('https://github.com/Brhiza/sydf/releases/download/v0.2.0/shiyue-dongfang-0.2.0-release.apk');
    expect(routes[2]?.url).toContain('gh-proxy.com/https://github.com/');
    expect(routes[3]?.url).toContain('ghfast.top/https://github.com/');
    expect(routes[4]?.url).toBe('https://sydf.cc/api/app-download?version=0.2.0');
  });
  ...
  it('自动选择延迟最低的可用线路，全部失败时回退到线路一', () => {
    ...
    expect(selectBestDownloadRoute(routes, [])?.id).toBe('lanzou-api');
  });
});
