...
describe('今日运势批量内容质量', () => {
  it('不同案例与周期不再出现错误状态、低信息套话或高频重复', () => {
    ...
    const genericPattern = /重大决定宜多留一道复核|其余事项按既定次序跟进即可|保持弹性即可|照常核实|避免小问题累积|条件未齐时保留调整空间|不需要全面回避|确认承载条件|反复打断.+连续性|多分配一档精力|可用它配合主线|主线卡住时.+恢复进度|取决于前置条件|前置条件是否|优势存在，但仍取决|问题集中在部分条件|最需要前置核对|是较好的落点|留出复核余地|六项综合排序|由五行对应数、主线位置和盘面参数合并得出|适合作为配合项|辅助线|辅助推进项|不是(?:当天|本月|全年)主线|形成一个可复核结果|不扩大范围|方位信号不集中|整体判断的可用性更稳定|整体主线(?:顺序)?不变|主线结论仍成立|非紧急健康安排多确认|非紧急项目可复核时间与准备事项|较大的形象改变或复杂安排可多考虑一天|先确认双方意愿、健康与现实条件|把普通事情做扎实|不建议临时启动重大决定|作为配合|(?:工作事业|学习成长|金钱合作|沟通关系|出行行动|身心状态)保持连续|逐步接上|维持稳定投入|暂作配合|只做必要维护|不增加变量|再承接已有进展|可以承接，但不抢占主线资源|身心状态(?:随后|再承接|适合接在|逐步接上|维持稳定投入|不增加变量|只做必要维护|不再加量)/;
    results.forEach((result) => {
      ...
      result.evidenceInsights.filter((item) => ['opportunity', 'caution', 'secondary'].includes(item.key)).forEach((item) => {
        ...
        if (item.key === 'opportunity') {
          expect(item.detail).toMatch(/责任|验收|交付|学习成果|输入|专注|资料|款项|对账|交易|付款|留痕|询价|事实|共识|沟通|关系|行程|路线|返程|完整休息|睡眠|进食|专注度/);
          ...
          expect(item.detail).toMatch(/等.+后|完整休息|承载条件/);
          expect(item.detail).toMatch(/负责人|可复述|金额|双方理解|路线|实际余量|分清事实/);
        }
      });
