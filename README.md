...
scripts/          主题资源生成与维护脚本
source-assets/    仅保存在本机的可重建素材原稿
vendor/           当前接入的 mingyu-core 本地包
```

占卜主题原稿统一放在 `source-assets/divination-themes/<主题 ID>/`，生成后的网站成品放在
`public/divination-themes/<主题 ID>/`。原稿体积较大，不提交到 Git；成品需随代码提交，保证部署后可直接使用。
